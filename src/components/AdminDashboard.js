...
export function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState("users");
  const roles = [
    { id: "1", rolename: "super_user" },
    { id: "2", rolename: "admin" },
    { id: "3", rolename: "user" },
  ];
  const dispatch = useDispatch();
  const users = useSelector(
    (state) => state.usersList?.userListData?.payload?.data?.users
  );
  const pendingUsers = useSelector(
    (state) => state.usersList?.pendingUsers?.payload?.data?.pending_users
  );

  // Local copies of the users lists (to avoid reloading the full list)
  const [localUsers, setLocalUsers] = useState([]);
  const [localPendingUsers, setLocalPendingUsers] = useState([]);

  // Whenever the Redux state changes, update our local copies.
  useEffect(() => {
    if (users) {
      setLocalUsers(users);
    }
  ...
  useEffect(() => {
    if (pendingUsers) {
      setLocalPendingUsers(pendingUsers);
    }
  }, [pendingUsers]);

  useEffect(() => {
    setLoading(true);
    // Initially load the user lists
    dispatch(userList()).finally(() => {
      setLoading(false);
    });
    dispatch(pendingUserList()).finally(() => {
  ...
  const approveUser = (e, user) => {
    dispatch(userApprove(user.id))
      .then((response) => {
        if (response?.status === 200) {
          showSuccessAlert("Success", "User approved successfully!");
          // Update local pending list without reloading the page.
          setLocalPendingUsers((prev) => prev.filter((u) => u.id !== user.id));
        } else {
          showErrorAlert("Error", "Failed to approve user.");
        }
      })
        ...
        if (result.isConfirmed) {
          ...
          dispatch(userRoleEdit(user))
            .then((response) => {
              if (response?.status === 200) {
                showSuccessAlert("Success", "User role has been edited successfully!");
                // Optionally update the local list if the role change affects UI
                setLocalUsers((prev) =>
                  prev.map((u) => (u.id === user.userid ? { ...u, role: user.role } : u))
                );
              } else {
                showErrorAlert("Error", "Failed to edit user role.");
              }
        ...
        if (result.isConfirmed) {
          dispatch(userDelete(user.id))
            .then((response) => {
              if (response?.status === 200) {
                showSuccessAlert("Deleted!", "User has been deleted.");
                // Remove the user from the local list without reloading:
        ...
        if (result.isConfirmed) {
          dispatch(userReject(user.id))
            .then((response) => {
              if (response?.status === 200) {
                showSuccessAlert("Rejected!", "User has been rejected successfully.");
                // Remove the user from the pending local list
                setLocalPendingUsers((prev) => prev.filter((u) => u.id !== user.id));
              } else {
                showErrorAlert("Error", "Failed to reject user.");
              }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // For filtering, you could either dispatch userList or filter localUsers.
    dispatch(userList(formValues));
    setFormValues({ email: "", role: "", company_name: "" });
    setShowModal(false);
  };
    ...
    <Container fluid className="w-90">
      ...
      <div className="row">
        ...
        {activeadminTab === "adminUsers" ? (
              ...
              {activeTab === "users" && (
                ...
                >
                  <table
                    ...
                    style={{ fontSize: 10, marginLeft: "7%", width: "90%" }}
                  >
                    <thead
                          ...
                          <tr key={user.id}>
                            ...
                            <td>
                              ...
                              <button
                                ...
                                onClick={(e) => onResetPass(user)}
                                style={{ border: 0 }}
                              >
                                <img src={resetPass} alt="reset" />
                              </button>
                            </td>
                          </tr>
                        ))}
              ...
              {activeTab === "pendingUsers" && (
                ...
                >
                  ...
                  >
                    ...
                    {loading ? (
                      <TableSkeleton />
                    ) : localPendingUsers && localPendingUsers.length > 0 ? (
                      <tbody>
                        {localPendingUsers.map((user, index) => (
                          <tr key={user.id}>
                            <td>{index + 1}</td>
                            <td>{user.full_name}</td>
                            <td>{user.email}</td>
          ...
          <div className="col-7 card" style={{ marginLeft: "16%" }}>
            <div className="d-flex gap-2 ms-5 mt-3">
              <Button className="btn btn-primary btn-sm" onClick={handleFlushDB}>
                Flush DB
              </Button>
              <Button className="btn btn-primary btn-sm" onClick={handleContainerRestart}>
                Restart Container
              </Button>
            ...
            <Row className="flex-column m-3 h-100">
              <Col className="d-flex flex-column align-items-center p-4 border rounded mb-3">
                ...
                <div className="d-flex align-items-start mt-4 w-100">
                  ...
                  <Button className="btn btn-primary btn-sm mt-2" onClick={handleLLMSubmit}>
                    Submit
                  </Button>
                </div>
              </Col>
              <Col className="d-flex flex-column align-items-center p-4 border rounded">
                ...
                <div className="d-flex align-items-start mt-4 w-100">
                  ...
                  <Switch checked={isActive} size="large" onChange={handleToggle} />
                </div>
              </Col>
              <Col className="d-flex flex-column align-items-center p-4 border rounded mt-3">
                <h5>Storage Location</h5>
                <div className="d-flex align-items-start mt-3 w-100 h-10">
                  ...
                  <Button className="btn btn-primary btn-sm mt-2" onClick={handleStorageClick}>
                    Submit
                  </Button>
                </div>
              </Col>
