...
const OffCanvas = ({
  ...
  const CustomInput = React.forwardRef(({ value, onClick }, ref) => (
    <input ref={ref} value={value} onClick={onClick} className="classlable" />
  ));
      ...
      <div className="offcanvas offcanvas-end show" tabIndex="-1">
        ...
        <div className="offcanvas-body">
          <form onSubmit={handleSubmit} className="classform">
            {selectedFilter.key !==
              GRAPHKEYS.ISSUSE_SEVERITY_FREQUESCY_PROJECT &&
              selectedFilter.key !== GRAPHKEYS.MONTH_USAGE && (
                <div>
                  <label htmlFor="filterSelect" className="formlabels">
                    Project Name
                  </label>
                  <select
                    id="filterSelect"
            ...
            {selectedFilter?.key !== GRAPHKEYS.ISSUE_SEVERITY_DISTRIBUTION &&
              selectedFilter?.key !==
                GRAPHKEYS.ISSUSE_SEVERITY_FREQUESCY_PROJECT &&
              selectedFilter.key !== GRAPHKEYS.MONTH_USAGE &&
              selectedFilter.key !== GRAPHKEYS.AVARAGE_CODE_QUALITY &&
              selectedFilter.key !== GRAPHKEYS.AVARAGE_CODE_SEVERITY && (
                <div>
                  <label htmlFor="userSelect" className="formlabels">
                    Developer
                  </label>
                  <select
                    id="userSelect"
            ...
            {(selectedFilter.key === GRAPHKEYS.COMMIT_AVARAGE_CODE_QUALITY ||
              selectedFilter.key === GRAPHKEYS.COMMIT_VIOLATE ||
              selectedFilter.key ===
                GRAPHKEYS.COMMIT_ISSUE_SEVERITY_BY_USER_PROJECT ||
              selectedFilter.key === GRAPHKEYS.COMMIT_ORG_COMMIT_METRICS) && (
                ...
                <DatePicker
                  ...
                  startDate={
                    selectedFilter.start_date
                      ? new Date(selectedFilter.start_date)
                      : ""
                  }
                  endDate={
                    selectedFilter.end_date
                      ? new Date(selectedFilter.end_date)
                      : ""
                  }
                  onChange={(dates) => {
                    const [start, end] = dates;
                    ...
                    const formattedEndDate = end
                      ...
                      : "";
                    onChange({
                      target: { name: "start_date", value: formattedStartDate },
                    });
                    onChange({
                      target: { name: "end_date", value: formattedEndDate },
                    });
                  }}
                  format="YYYY-MM-DD"
            ...
            {(selectedFilter.key ===
              GRAPHKEYS.ISSUSE_SEVERITY_FREQUESCY_PROJECT ||
              selectedFilter.key === GRAPHKEYS.REVIEW_USAGE_DATA ||
              selectedFilter.key === GRAPHKEYS.ASSIANCE_USAGE_DTA ||
              selectedFilter.key === GRAPHKEYS.MONTH_USAGE) && (
