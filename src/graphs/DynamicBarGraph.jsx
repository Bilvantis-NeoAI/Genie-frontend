...
const DynamicBarGraph = ({ title, data, keys, handleFilter }) => {
  ...
  if (title === TITLE.COMMIT_ISSUES_SEVERITY) {
    ...
    bars = [
      {
        key: DATAKEY.DISABLE_FILES_COUNT,
        color: "#1DB9EF",
        name: XAXISNAMES.DISABLE_FILES_COUNT,
      },
      {
        key: DATAKEY.SECRETS_COUNT,
        color: "#1DEF81",
        name: XAXISNAMES.SECRETS_COUNT,
      },
    ];
  }
      ...
      <MouseEventsHandler>
        {({
          ...
          <div
            ...
            onMouseDown={data.length > 0 ? handleMouseDown : undefined}
            onMouseMove={data.length > 0 ? handleMouseMove : undefined}
            onMouseUp={data.length > 0 ? handleMouseUp : undefined}
            onMouseLeave={data.length > 0 ? handleMouseUp : undefined}
          >
            {data.length === 0 ? (
