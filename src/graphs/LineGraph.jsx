...
const LineGraph = ({ data, title, handleFilter, from, keys }) => {
  ...
  const renderLegend = () => {
    if (
      ...
      return (
        <span
          style={{ color: "#1DB9EF", fontSize: "12px", marginRight: "10px" }}
        >
  ...
  return (
    <div className="card g-4">
      <div className="graph-title">
        ...
        <div>
          <button
            ...
            onClick={async () => {
              try {
                await handleFilter(data, title, keys);
              } catch (err) {
                alert(
                  "Something went wrong while applying filters. Please try again."
                );
              }
            }}
      ...
      <div className="legend-labels">{renderLegend()}</div>
    </div>
  );
};
